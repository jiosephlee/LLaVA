...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if images is not None and smiles is not None:
            raise ValueError("Cannot process both images and SMILES strings at the same time.")
            ...
            if type(images) is list or images.ndim == 5:
                ...
                if mm_patch_merge_type == 'flat':
                    ...
                    for image_idx, image_feature in enumerate(multimodal_features):
                        ...
                        new_multimodal_features.append(image_feature)
                    multimodal_features = new_multimodal_features
                else:
        ...
        if attention_mask is not None:
            assert new_input_embeds.shape[1] == attention_mask.shape[1], (
                f"len mismatch: embeds={new_input_embeds.shape}, mask={attention_mask.shape}"
            )
        if position_ids is not None:
            assert new_input_embeds.shape[1] == position_ids.shape[1], (
                f"len mismatch: embeds={new_input_embeds.shape}, pos={position_ids.shape}"
            )
        if new_labels is not None:
