...
#SBATCH --output=logs/llava_pretrained_100k_smiles_mid_100k_alignment.out
#SBATCH --error=logs/llava_pretrained_100k_smiles_mid_100k_alignment.err
...
ALIGNMENT_OUTPUT_DIR="checkpoints/pretrain_100k_mid_stage_alignment_100k_smiles"
...
if [ -n "${ALIGNMENT_CHECKPOINT}" ]; then
...
elif [ -f "${ALIGNMENT_OUTPUT_DIR}/pytorch_model.bin" ] || [ -f "${ALIGNMENT_OUTPUT_DIR}/adapter_config.json" ]; then
  echo "➤ Using Alignment Stage output directory as checkpoint"
  FINETUNE_MODEL_PATH="${ALIGNMENT_OUTPUT_DIR}"
  ...
  FINETUNE_PROJECTOR_PATH="checkpoints/pretrain_100k/mm_projector.bin"
fi
